...
const uint8_t TEXT_TO_SEND[] = {"STM32 DMA Usart Test"};
#define SEND_BUF_SIZE (sizeof(TEXT_TO_SEND) + 2) * 10
...
//uint8_t key = 0;
  ...
  MX_ADC1_Init();
  /* USER CODE BEGIN 2 */
  circle_buffer_init(&g_key_bufs, 100, g_data_buf);
  //Green_LED_ON();
  /* USER CODE END 2 */
  ...
  while (1)
  {
    // adcx = adc_get_result_average(ADC_CHANNEL_1, 10);
